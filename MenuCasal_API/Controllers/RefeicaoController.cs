...
{
    private readonly MenuContext _dbContext;

    public RefeicaoController(MenuContext dbContext)
    {
    ...
    {
        if (_dbContext.Refeicoes.Count() > 0)
            return _dbContext.Refeicoes.ToList();
        else
    ...
    [HttpPost]
    public int Post(IEnumerable<Refeicao> request)
    {
        _dbContext.Refeicoes.AddRange(request);
